                ...
                while (addIt.next()) {
                    if (properties.has(addIt.key)) {
                        ...
                        if ((declaredProperties[addIt.key] && declaredProperties[addIt.key].type !== 'color'
                            ...
                            && declaredProperties[addIt.key].type !== 'datalist' && declaredProperties[addIt.key].type !== 'multiselect')
...
Inspector.prototype.buildPropertyRow = function (propertyName, propertyValue) {
    ...
    if (propertyName == 'parentToolUrl') {
        tr.setAttribute("hidden","");
    }
    if (propertyName == 'kubeConfigIdList') {
        tr.setAttribute("hidden", "");
    }
    ...
    function updateurl() { self.updateUrlProperties(); }

    if (decProp && decProp.type === "multiselect") {
        input = document.createElement("select");
        input.setAttribute("multiple", "multiple");
        this.updateSelect(decProp, input, propertyName, propertyValue);
        input.addEventListener("change", updateall);
    }
    else if (decProp && decProp.type === "select") {
        input = document.createElement("select");
        this.updateSelect(decProp, input, propertyName, propertyValue);
        //if (propertyName == 'parentToolUrl') {
    ...
    else if (decProp && decProp.type === "datalist") {
        input = document.createElement("datalist");
        this.updateSelect(decProp, input, propertyName, propertyValue);
        if (propertyName == 'parentToolUrl') {
            input.id = "parentToolUrllist";
            input.addEventListener("change", updateall);
        }
        else if (propertyName == 'kubeConfigIdList') {
            input.id = "kubeConfigIdList";
            input.addEventListener("change", updateall);
        }
        else {
            input.addEventListener("change", updateall);
    ...
    else if (propertyName == 'kubernetesconfigid') {
        input = document.createElement("input");

        input.value = this.convertToString(propertyValue);
        //input.setAttribute("type", "text");
        input.setAttribute("list", "kubeConfigIdList");
        input.addEventListener("change", updateall);
    }
        ...
        input.value = this.convertToString(propertyValue);
        //input.setAttribute("type", "text");
        input.setAttribute("list", "parentToolUrllist");
        input.addEventListener("change", updateall);
    }
    else {
        input = document.createElement("input");
        ...
        if (input.type !== "color" && propertyName !== 'credentialsId' && propertyName !== 'username'
...
Inspector.prototype.updateSelect = function (decProp, select, propertyName, propertyValue) {
    ...
    if (propertyValue.constructor === Array) {
        for (var i = 0; i < select.options.length; i++) {
            select.options[i].selected = propertyValue.indexOf(select.options[i].value) >= 0;
        }
    }
    else {
        var propValue = this.convertToString(propertyValue);
        if (propValue.includes(',')) {
            var propArray = propValue.split(',');
            propValue = propArray[0];
        }
    ...
    if (diagram.selection.count === 1 || !this.multipleSelection) { // single object update
        ...
        for (var name in inspectedProps) {
            ...
            switch (type) {
                ...
                case 'multiselect': value = SelectDropdownValues(input); break;
                case 'select': value = decProp.choicesArray[input.selectedIndex]; break;
                case 'datalist': value = decProp.choicesArray; break;
            }
...
Inspector.prototype.updateUrlProperties = function () {
    var inspectedProps = this._inspectedProperties;
    var diagram = this._diagram;
    ...
    diagram.startTransaction('set url properties');
    var purl = inspectedProps['parentToolUrl'];
    var purlData = purl.value;
    if (purlData !== '-Select-') {
        for (var name in inspectedProps) {
            if (name == 'url') {
                var input = inspectedProps[name];
                var decProp = this.declaredProperties['parentToolUrl'];
                purlData = decProp.choicesArray[purl.selectedIndex];
                diagram.model.setDataProperty(data, 'parentToolUrl', purlData);
                // notify any listener
                if (this.propertyModified !== null) this.propertyModified('parentToolUrl', purlData, this);



                // modify the data object in an undo-able fashion

                diagram.model.setDataProperty(data, name, purlData);

                // notify any listener
                if (this.propertyModified !== null) this.propertyModified(name, purlData, this);
            }
        }
    }
    diagram.commitTransaction('set url properties');



};

function SelectDropdownValues(ddlData) {

    var str = [], i;

    for (i = 0; i < ddlData.selectedOptions.length; i++) {
        str.push(ddlData.selectedOptions[i].value);
    }
    return str;
}